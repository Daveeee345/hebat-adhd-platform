...
export default function ChildSupportPlan({ currentRole, setPage }:{ currentRole:'parent'|'child'|'teacher'|'professional'; setPage:(p:Page)=>void }){
  ...
  return <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-10 space-y-7">
    ...
    <section className="card-pillowy">
      ...
      <div className="flex flex-wrap gap-3 mt-5"><button onClick={()=>setPage('screening-intro')} className="btn-outline !py-2.5 text-xs">{isId?'Ulangi Check-in':'Retake Check-in'}</button>{data.screenings.length>0&&<span className="rounded-xl bg-neutral-100 px-4 py-2.5 text-xs font-black text-neutral-500">{data.screenings.length} {isId?'check-in tersimpan':'saved check-in(s)'}</span>}</div>
    </section>
